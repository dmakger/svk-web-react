...
const Article = ({title, linkVideo, description, writer, brand, tags, image, dateAt, content, className, classNameText, ...props}) => {
    return (
        <div className={`${cl.article} ${className}`} {...props}>
            ...
            <ArticleContent image={image}
                            content={content}
                            linkVideo={linkVideo}
                            className={cl.content} classNameText={classNameText}/>
        </div>
    );
};
